...
| C-15 | Screen review | PROPOSED (approved as a batch) | Task search (title + description, wildcards escaped); recurring tasks reworded in plain language ("משימה חוזרת", "הפעם הבאה", "הפסקה") with an edit form (name, details, owner, participants, end date — next times only); archived documents tagged on the task page. |
